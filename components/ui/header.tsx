...
export default function Header() {
  ...
  const fetchUser = async () => {
    try {
      ...
      if (user) {
        ...
        // Fetch notification count (placeholder - implement based on your notification system)
        // setNotifications(5)
        // setMessages(3)
      }
    } catch (error) {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      // For now, just return user results. Post search can be implemented when posts table is ready
      setSearchResults(userResults)
      setShowSearchResults(true)
    } catch (error) {
      console.error('Search error:', error)
      // Fallback to just user search if posts table doesn't exist
      try {
        const { data: users } = await supabase
          .from('users')
          .select('id, username, full_name, profile_image')
  ...
  const navigation = [
    ...
    { icon: UserIcon, label: 'Profile', href: userProfile ? `/profile/${userProfile.username}` : '/profile' },
    { icon: MessageCircle, label: 'Messages', href: '/messages', badge: messages },
            ...
            >
              <div className="w-8 h-8 rounded-lg flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 512 512" fill="none">
                  ...
                  <circle cx="256" cy="256" r="240" fill="none" stroke="url(#glowGradient)" strokeWidth="2" opacity="0.6"/>
                  ...
                  <defs>
                    ...
                    <linearGradient id="glowGradient" x1="16" y1="256" x2="496" y2="256" gradientUnits="userSpaceOnUse">
                      ...
                      <stop offset="0.5" stopColor="#F6C561" stopOpacity="0.7"/>
            ...
            <div className="relative ml-3" ref={profileRef}>
              ...
              >
                {userProfile?.profile_image ? (
                  ...
                  <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: '#070B1C' }}>
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 mt-2 w-56 bg-card/95 backdrop-blur border border-border rounded-lg shadow-lg py-1 z-50">
                  ...
                  <button
                    onClick={() => {
                      router.push(userProfile ? `/profile/${userProfile.username}` : '/profile')
                      setShowProfileDropdown(false)
                    }}
